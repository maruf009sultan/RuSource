...
export const Route = createFileRoute("/learn/$topic")({
  ...
  head: ({ loaderData, params }) => ({
    meta: loaderData ? [
      ...
      { property: "og:url", content: absUrl(`/learn/${params.topic}`) },
      { property: "og:type", content: "website" },
    ] : [],
    links: [{ rel: "canonical", href: absUrl(`/learn/${params.topic}`) }],
    scripts: loaderData ? [
      {
        ...
        children: JSON.stringify({
          ...
          itemListElement: [
            ...
            { "@type": "ListItem", position: 2, name: loaderData.title, item: absUrl(`/learn/${params.topic}`) },
          ],
        }),
      },
      {
    ...
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <article className="prose prose-neutral mt-12 max-w-3xl text-lg leading-relaxed text-foreground/90 dark:prose-invert">
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          ...
          {topic === "free" && <>
            <Link to="/learn/$topic" params={{ topic: "podcasts" }} className="border border-ink/30 bg-card px-2 py-1 font-mono text-xs hover:border-signal hover:text-signal">Free podcasts</Link>
            <Link to="/learn/$topic" params={{ topic: "apps" }} className="border border-ink/30 bg-card px-2 py-1 font-mono text-xs hover:border-signal hover:text-signal">Free apps</Link>
            <Link to="/learn/$topic" params={{ topic: "beginners" }} className="border border-ink/30 bg-card px-2 py-1 font-mono text-xs hover:border-signal hover:text-signal">Beginner resources</Link>
          </>}
          {topic === "podcasts" && <>
            <Link to="/learn/$topic" params={{ topic: "free" }} className="border border-ink/30 bg-card px-2 py-1 font-mono text-xs hover:border-signal hover:text-signal">All free resources</Link>
            <Link to="/category/$slug" params={{ slug: "youtube-video" }} className="border border-ink/30 bg-card px-2 py-1 font-mono text-xs hover:border-signal hover:text-signal">YouTube channels</Link>
          </>}
          {topic === "apps" && <>
            <Link to="/learn/$topic" params={{ topic: "free" }} className="border border-ink/30 bg-card px-2 py-1 font-mono text-xs hover:border-signal hover:text-signal">All free resources</Link>
            <Link to="/category/$slug" params={{ slug: "flashcards-srs" }} className="border border-ink/30 bg-card px-2 py-1 font-mono text-xs hover:border-signal hover:text-signal">Flashcards & SRS</Link>
          </>}
          {topic === "beginners" && <>
            <Link to="/levels/$level" params={{ level: "A1" }} className="border border-ink/30 bg-card px-2 py-1 font-mono text-xs hover:border-signal hover:text-signal">A1 resources</Link>
            <Link to="/category/$slug" params={{ slug: "alphabet-phonetics" }} className="border border-ink/30 bg-card px-2 py-1 font-mono text-xs hover:border-signal hover:text-signal">Alphabet</Link>
            <Link to="/learn/$topic" params={{ topic: "free" }} className="border border-ink/30 bg-card px-2 py-1 font-mono text-xs hover:border-signal hover:text-signal">Free resources</Link>
          </>}
          {topic === "grammar" && <>
            <Link to="/glossary" className="border border-ink/30 bg-card px-2 py-1 font-mono text-xs hover:border-signal hover:text-signal">Glossary</Link>
            <Link to="/category/$slug" params={{ slug: "writing-grammar-tools" }} className="border border-ink/30 bg-card px-2 py-1 font-mono text-xs hover:border-signal hover:text-signal">Writing tools</Link>
          </>}
          {topic === "online" && <>
            <Link to="/learn/$topic" params={{ topic: "free" }} className="border border-ink/30 bg-card px-2 py-1 font-mono text-xs hover:border-signal hover:text-signal">Free resources</Link>
            <Link to="/roadmap" className="border border-ink/30 bg-card px-2 py-1 font-mono text-xs hover:border-signal hover:text-signal">Roadmap</Link>
          </>}
        </div>
      </article>
